...
def query_freight_value_weight_relationship(database: Engine) -> QueryResult:
    ...
    # inside that order.
    # Use the pandas DataFrame stored in `delivered` variable.
    # Store the result in the `aggregations` variable.
    aggregations = (
        delivered.groupby("order_id")
        .agg({"product_weight_g": "sum", "freight_value": "sum"})
        .reset_index()
    )
...
def query_orders_per_day_and_holidays_2017(database: Engine) -> QueryResult:
    ...
    order_purchase_ammount_per_date = filtered_dates.groupby(filtered_dates["order_purchase_timestamp"].dt.date).count()
